...
public final class GameBoard {
    ...
    public String playGame(Player x, Player y) {
        ...
        if (finish == 'B') {
            Battleground.addPoints(x);
            return "JUGADOR " + x.getName() + " HA GANADO A JUGADOR " + y.getName() + ", FELICIDADES HA GANADO 3 PUNTOS";
        } else {
            Battleground.addPoints(y);
            return "JUGADOR " + y.getName() + " HA GANADO A JUGADOR " + x.getName() + ", FELICIDADES HA GANADO 3 PUNTOS";
        }
    }
    ...
    private void attackZombie(Pieza p) {
        boolean atk = false;
        do {
            Pieza pZombie = validarPiezaSeleccionada("Z" + Character.toString(p.getColor()));
            Pieza pEnemy;
            do {
                int coordinates[] = ingresarFilasColumnas();
                if (chess[coordinates[0]][coordinates[1]] == null) {
    ...
    private void attackSpecialPiezaEnemiga(Pieza p) {
        boolean atk = false;
        if (p instanceof Necromancer) {
            ((Necromancer) p).updateSpecial();
        }
        do {
            int coordinates[] = ingresarFilasColumnas();
            if (chess[coordinates[0]][coordinates[1]] == null) {
        ...
        if (p instanceof Necromancer) {
            ((Necromancer) p).updatePosiciones(p.row, p.column);
        }
    }

    private void createZombie(Pieza p) {
        boolean crear = false;
        if (p instanceof Necromancer) {
            do {
                int coordinates[] = ingresarFilasColumnas();
                if (chess[coordinates[0]][coordinates[1]] == null) {
                    chess[coordinates[0]][coordinates[1]] = ((Necromancer) p).createZombie(coordinates[0], coordinates[1]);
                    piezas[coordinates[0]][coordinates[1]] = "Z" + Character.toString(p.getColor());
                    crear = true;
                    //System.out.print("Z Creado");
                }
            } while (!crear);
        }
    }
    ...
    /**
     * Ingresa la fila y columna a utilizar (mover o atacar o crear en caso de
     * Necromancer).
     *
    ...
    /**
     * Determina si el jugador contrario no tiene piezas estableciendo el fin de
     * juego
     *
     * @return
     */
